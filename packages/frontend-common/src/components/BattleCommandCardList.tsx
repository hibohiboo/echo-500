...
interface BattleCommandCardListProps {
  cards: BattleCommandCardData[];
  loading?: boolean;
  onCardClick?: (card: BattleCommandCardData) => void;
}
...
export function BattleCommandCardList({
  ...
  loading = false,
  onCardClick,
}: BattleCommandCardListProps) {
  return (
    <div className="battle-command-card-list">
      <div className="battle-command-card-list__header">
        <h2 className="battle-command-card-list__title">戦闘用コマンド一覧</h2>
        {cards.length > 0 && (
          <span className="battle-command-card-list__count">
            {cards.length} cards
          </span>
        )}
      </div>
      ...
      {loading ? (
        <div className="battle-command-card-list__loading">
          <div className="battle-command-card-list__spinner" />
          <p>Loading battle commands...</p>
        </div>
      ) : (
        <div className="battle-command-card-list__grid">
