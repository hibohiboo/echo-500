...
function ActionButtons({
  ...
  if (!onExecute && !onDetails) return null;

  return (
    <div className="inventory-card__actions">
      {onExecute && (
        <button
          className="inventory-card__action-btn inventory-card__action-btn--execute"
          onClick={(e) => {
            e.stopPropagation();
            onExecute?.();
          }}
      ...
      {onDetails && (
        <button
          className="inventory-card__action-btn inventory-card__action-btn--details"
          onClick={(e) => {
            e.stopPropagation();
            onDetails?.();
          }}
...
export function InventoryCard({
  ...
  return (
    <div className="inventory-card" onClick={onClick}>
      <div
        className="inventory-card__border"
        style={{
          borderColor,
          boxShadow: `0 0 10px ${borderColor}40`,
        }}
      >
        <div className="inventory-card__type-badge">{typeLabels[type]}</div>

        <div className="inventory-card__content">
          {icon && (
            <div className="inventory-card__icon">
              <InventoryIcon icon={icon} size="medium" />
            </div>
          )}
          <div className="inventory-card__name">{name}</div>
        </div>

        <div className="inventory-card__circuit">
        ...
        <ActionButtons onExecute={onExecute} onDetails={onDetails} />
      </div>
