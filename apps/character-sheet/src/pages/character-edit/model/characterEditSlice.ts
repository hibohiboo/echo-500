...
export const characterEditSlice = createSlice({
  name: 'characterEdit',
  initialState,
  reducers: {
    ...
    setMemorySlots: (state, action: PayloadAction<GraphDbMemoryNode[]>) => {
    ...
    setSelectedBattleCommands: (
      ...
      state.selectedBattleCommands = action.payload;
    },
    resetForm: (state) => {
      state.id = '';
      state.name = '';
    ...
    addMemorySlot: (state, action: PayloadAction<GraphDbMemoryNode>) => {
