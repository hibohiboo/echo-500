...
import { isIncluceImage } from './useExportScenario';
  ...
  const importScenario = useCallback(
    async (file: File) => {
      try {
        ...
        const scenarioData = parseImportData(JSON.parse(scenarioJsonText));
        ...
        if (isIncluceImage) {
          ...
          const images = await Promise.all(
            imageEntries.map(async (entry) => {
              ...
              const imageMetadata = scenarioData.rdb.images.find(
                (img) => img.id === imageId,
              );
              if (!imageMetadata) {
                return null;
          ...
          await dispatch(
            importScenarioAction({
              graphData: scenarioData.graphdb,
              rdbData: {
                scenario: scenarioData.rdb.scenario,
                images: validImages,
              },
            }),
          ).unwrap();
