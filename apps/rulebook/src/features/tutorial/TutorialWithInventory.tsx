...
export function TutorialWithInventory() {
  const [visibleSteps, setVisibleSteps] = useState<string[]>(['step1']);
    ...
    },
    {
      ...
      details: (
        <>
          ...
          <p style={{ fontFamily: 'monospace', color: '#ff6b6b' }}>
  ...
  const [toastQueue, setToastQueue] = useState<InventoryItem[]>([]);
    ...
    <div className="tutorial-container">
              ...
              >
                {step.commands.map((command) => (
                  <CommandCard
                    ...
                    disabled={
                      selectedCommand !== null && selectedCommand !== command.id
                    }
      ...
      {toastQueue.map((item) => (
        <ItemToast
          key={item.id}
          item={item}
          onAnimationEnd={() => removeToast(item)}
        />
      ))}
